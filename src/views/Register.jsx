...
const ClearButton = ({ field, form, setForm }) => (
  form[field] ? (
    <button type="button" onClick={() => setForm((prev) => ({ ...prev, [field]: "" }))} className="text-gray-400 hover:text-gray-600">
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <div className="px-7 pb-7">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {/* Full Name */}
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block" style={{ fontFamily: "'Poppins', sans-serif" }}>Full Name</label>
              <div className="flex items-center border border-gray-200 rounded-lg px-3 py-2 gap-2 focus-within:border-green-600 transition-colors">
                ...
                <input type="text" name="fullName" placeholder="Juan Dela Cruz" value={form.fullName} onChange={handleChange}
                  className="flex-1 text-sm outline-none bg-transparent text-gray-700 placeholder-gray-400"
                  style={{ fontFamily: "'Poppins', sans-serif" }} />
                <ClearButton field="fullName" form={form} setForm={setForm} />
              </div>
            </div>

            {/* Contact Number */}
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block" style={{ fontFamily: "'Poppins', sans-serif" }}>Contact Number</label>
              <div className="flex items-center border border-gray-200 rounded-lg px-3 py-2 gap-2 focus-within:border-green-600 transition-colors">
                <svg className="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <input type="text" name="contactNumber" placeholder="(+63) 945 6789" value={form.contactNumber} onChange={handleChange}
                  className="flex-1 text-sm outline-none bg-transparent text-gray-700 placeholder-gray-400"
                  style={{ fontFamily: "'Poppins', sans-serif" }} />
                <ClearButton field="contactNumber" form={form} setForm={setForm} />
              </div>
            </div>

            {/* Email */}
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block" style={{ fontFamily: "'Poppins', sans-serif" }}>Email Address</label>
              <div className="flex items-center border border-gray-200 rounded-lg px-3 py-2 gap-2 focus-within:border-green-600 transition-colors">
                <svg className="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <input type="email" name="emailAddress" placeholder="juan@mytalipapa.ph" value={form.emailAddress} onChange={handleChange}
                  className="flex-1 text-sm outline-none bg-transparent text-gray-700 placeholder-gray-400"
                  style={{ fontFamily: "'Poppins', sans-serif" }} />
                <ClearButton field="emailAddress" form={form} setForm={setForm} />
              </div>
            </div>
